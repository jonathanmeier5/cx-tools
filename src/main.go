...
func main() {
	...
	key := strings.Join(objectParts[1:], "/")
	profile := os.Args[2]
	fmt.Printf("creating signed upload url for bucket=%s, object path=%s\n", bucket, key)

	cfg, err := config.LoadDefaultConfig(context.TODO(), config.WithRegion(AWS_S3_REGION), config.WithSharedConfigProfile(profile))
	if err != nil {
		log.Fatal(err)
	}
	...
	p := Presigner{}
	p.PresignClient = s3.NewPresignClient(awsS3Client)
	presignedPutRequest, err := p.PutObject(bucket, key, 12*60*60)
	if err != nil {
		fmt.Print(err.Error())
	}
	
	fmt.Println(presignedPutRequest.URL)
